...
from examples.multitask.loaders import make_loaders
...
class Trainer(Experimentalist):
	...
	def iteration_metrics(self,loss,lower_loss,acc ):
		...
		upper_grad_norm = torch.norm(torch.stack([torch.norm(var.grad) if var.grad is not None else torch.zeros([1]) for var in  self.upper_var],axis=0)).detach().item()
